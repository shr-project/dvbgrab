...
switch ($_GET["action"]) {
	...
	case "grab_add":
		...
		if ($row = $rs->FetchRow()) {
			...
			if (get_user_grab($usr_id, $DB->UserDate($row[0],"W"))>= $grab_quota) {
				header("Location:$PHP_SELF?query=$query&msg=grb_add_fail_quota&tv_date=$tv_date#$tel_id");
				exit;
			}

			// pozadavek o grab na uz odvysilany porad
			if ($DB->UnixTimeStamp($row[0])<$grab_time_limit) {
				header("Location:$PHP_SELF?query=$query&msg=grb_add_fail_time&tv_date=$tv_date#$tel_id");
				exit;
			}
			...
			if ($row[2]) {
				...
				header("Location:$PHP_SELF?query=$query&msg=grb_add_ok&tv_date=$tv_date#$tel_id");
				exit;
		...
		// porad s $tel_id neexistuje
			...
			header("Location:$PHP_SELF?query=$query&msg=grb_add_fail_tel&tv_date=$tv_date");
			exit;
		}
		break;
	...
	case "grab_add_me":
		...
		if ($row = $rs->FetchRow()) {
			...
			if (get_user_grab($usr_id, $DB->UserDate($row[1],"W"))>= $grab_quota) {
				header("Location:$PHP_SELF?query=$query&msg=grb_add_fail_quota&tv_date=$tv_date#$grb_id");
				exit;
			}

			// pozadavek o grab na uz odvysilany porad
			if ($DB->UnixTimeStamp($row[1])<$grab_time_limit) {
				header("Location:$PHP_SELF?query=$query&msg=grb_add_fail_time&tv_date=$tv_date#$grb_id");
				exit;
			}
			...
			header("Location:$PHP_SELF?query=$query&msg=grb_add_ok&tv_date=$tv_date#$grb_id");
			exit;
  ...
  case "grab_noenc":
    ...
    db_sql($SQL);
    status_update();
		header("Location:$PHP_SELF?query=$query&msg=grb_noenc_ok&tv_date=$tv_date#$grb_id");
    exit;
    break;
  ...
  case "grab_enc":
    ...
    db_sql($SQL);
    status_update();
	  header("Location:$PHP_SELF?query=$query&msg=grb_enc_ok&tv_date=$tv_date#$grb_id");
    exit;
    break;
	...
	case "grab_del":
		...
		if ($row = $rs->FetchRow()) {
			...
			if ($DB->UnixTimeStamp($row[1])<$grab_time_limit) {
				header("Location:$PHP_SELF?query=$query&msg=grb_del_fail_time&tv_date=$tv_date#$row[0]");
				exit;
			}
			...
			header("Location:$PHP_SELF?query=$query&msg=grb_del_ok&tv_date=$tv_date#$row[0]");
			exit;
		
		// grab s $grb_id neexistuje
		} else {
			header("Location:$PHP_SELF?query=$query&msg=grb_del_fail_exist&tv_date=$tv_date");
			exit;
		}
		break;
