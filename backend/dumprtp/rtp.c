...
int makeclientsocket(char *szAddr,unsigned short port,int TTL,struct sockaddr_in *sSockAddr) {
  int rmem_size = 20*1024*1024;
  socklen_t rmem_size_len = sizeof(rmem_size);
  ...
  if (setsockopt(socket,SOL_SOCKET,SO_RCVBUF,&rmem_size,sizeof(rmem_size))) {
    perror("set rmem size");
  }
  if (getsockopt(socket,SOL_SOCKET,SO_RCVBUF,&rmem_size,&rmem_size_len)) {
    perror("get rmem size");
  }
  else {
    fprintf(stderr, "rmem: %d\n", rmem_size);
  }
