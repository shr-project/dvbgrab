...
*/
function show_television($tel_id, $tel_date_start, $tel_name, $tel_desc, $tel_category, $grb_id, $req_status, $my_grab, $query="", $show_logo, $chn_logo, $chn_name,$tv_date, $show_link, $last=false) {
  echo "<tr";
  ...
  if(!empty($tel_category)) {
    echo '<span class="programCategory">'.$tel_category.'</span><br />';
  }
  echo '<span class="programDesc">'.htmlspecialchars($tel_desc).'</span>';

  echo "\n<br />\n";
  if (show_grab_del_link($grb_id, $req_status, $my_grab, $query)) {
    echo "\n<br />\n";
  }
  if ($show_link) {
    echo "<a href=\"tvprog.php?tv_date=".$tv_date."#".$tel_id."\">"._MsgGrabLinkShow."</a>";
  }
  echo "</td>\n</tr>\n";
}
...
function show_television_row($row, $query, $highlight_strings, $use_diacritics, $show_link, $show_logo, $last=false) {
  ...
  if (!empty($highlight_strings)) {
    $tel_name = str_match_array_ascii(htmlspecialchars($tel_name),
        $highlight_strings, "<span class=\"match\">$1</span>", $use_diacritics);
  }
  $tel_desc = $row["tel_desc"];
  if (!empty($highlight_strings)) {
    $tel_desc = str_match_array_ascii(htmlspecialchars($tel_desc),
        $highlight_strings, "<span class=\"match\">$1</span>", $use_diacritics);
  }
  $chn_name = $row["chn_name"];
  $chn_logo = $row["chn_logo"];

  show_television($tel_id, $tel_date_start, $tel_name, $tel_desc, $tel_category, $grb_id, $req_status, $my_grab, $query, $show_logo, $chn_logo, $chn_name, $tv_date, $show_link, $last);
}
...
//  echo "tel_id: $tel_id, tel_date_start: $tel_date_start, grab_time_limit: $grab_time_limit, text: $text, my_grab: $my_grab, query: $query";
  ...
  if ($tel_date_start >= $grab_time_limit) {
    echo "<a onclick=\"return confirm('"._MsgGrabConfirmStart." ".strip_tags($text)." "._MsgGrabConfirmGrab."')\" ".
      ...
      " title=\""._MsgGrabLinkGrab."\" class=\"programName\">".
      $text."</a>";
  }
  else {
    echo $text;
...
*/
function show_grab_del_link($grb_id, $req_status, $my_grab, $query) {
  $result = false;
  if ($req_status == 'scheduled' && $grb_id && $my_grab) {
    echo "<a class=\"programDel\" href=\"$PHP_SELF?action=grab_del".
